...
import http.api.{ApiController, ApiRequest}
import javax.inject.Inject
import play.api.i18n.I18nSupport
import play.api.libs.json.JsValue
import play.api.mvc._
import models.$model;format="Camel"$

// NOTE: Add the following to conf/routes to enable compilation of this class:
/*
GET       /$model;format="lower,hyphen"$        http.controllers.$model;format="Camel"$Controller.index
GET       /$model;format="lower,hyphen"$        http.controllers.$model;format="Camel"$Controller.show($model;format="camel"$Id:Int)
POST      /$model;format="lower,hyphen"$        http.controllers.$model;format="Camel"$Controller.store
PUT       /$model;format="lower,hyphen"$        http.controllers.$model;format="Camel"$Controller.update($model;format="camel"$Id:Int)
DELETE    /$model;format="lower,hyphen"$        http.controllers.$model;format="Camel"$Controller.delete($model;format="camel"$Id:Int)
*/
